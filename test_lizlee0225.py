...
def f03(*words):
    truth = "{}".format(words)  # This is broken.
...
def f06(string1, string2):
    ...
    if len1 > len2:
        ...
        utput = len1/len2 * 100
        perc = round(output, 2)
    print("'{}' is longer than '{}' by {}".format(longer, shorter, diff))
    print("'{}' has only {} the number of chars of {}".format(shorter, perc, longer))
...
def f09():
    n = list(range(501))
    sum_all = [x for x in n if x%3 == 0 or x%5 == 0]
    return sum(sum_all)


###############################################################################
def f10(n=0):
    sum_lst = list(range(501))
    sum_all = []
    if sum_lst[0] % 3 == 0 or sum_lst[n] % 5 == 0:
        sum_all.append(sum_lst[0])
    n+=1
    f10(n)
    return sum(sum_all)
